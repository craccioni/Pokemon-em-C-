...
#include <locale.h>
#define MAX 300
//https://pt.stackoverflow.com/questions/250309/dica-de-como-transformar-la%C3%A7os-em-fun%C3%A7%C3%A3o-recursiva
...
//------------------------------------------------------------------------------------------PROTÓTIPOS DE FUNÇÃO-----------------------------------------------------------------------------------------
			...
			void mostraPokemons(int i, POKEBOLA *pokemon);
...
void mostraPokemons(int i, POKEBOLA *pokemon) {
	if (i == 0) 
		...
		int proximo = i+1;
		return i * mostraPokemons(proximo);
		printf("\n\n\t\t\tPokemon: %s\n\n",pokemon[i].nome);
		printf("\t\t\t[Peso: %.2f]\n",pokemon[i].peso);
		printf("\t\t\t[Altura: %.2f]\n",pokemon[i].altura);
...
i=0;
mostraPokemons(POKEBOLA *pokemon);
